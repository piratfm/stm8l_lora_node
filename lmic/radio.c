...
static void configChannel () {
    ...
    //uint32_t frf = muldiv32u(LMIC.freq, ((uint32_t)(1 << 19)), 32000000);
    uint32_t frf = muldiv32u(LMIC.freq, 524288, 32000000);
    //uint32_t frf = muldiv32u(LMIC.freq, 256, 15625);
    writeReg(RegFrfMsb, (u1_t)(frf>>16));
    writeReg(RegFrfMid, (u1_t)(frf>> 8));
    writeReg(RegFrfLsb, (u1_t)(frf>> 0));  
#if LMIC_DEBUG_LEVEL > 2
    lmic_printf("%lu: configChannel(%lu): %02x %02x %02x ||| %02x %02x %02x\n",
                os_getTime(), LMIC.freq,
                (u1_t)(frf>>16), (u1_t)(frf>> 8), (u1_t)(frf>> 0),
                readReg(RegFrfMsb), readReg(RegFrfMid), readReg(RegFrfLsb));
#endif
}
...
static void configPower () {
    ...
    //writeReg(RegPaConfig, (u1_t)(0x80|0x70|(pw&0xf)));
    writeReg(RegPaConfig, (u1_t)(0x80|(pw&0xf)));
    writeReg(RegPaDac, readReg(RegPaDac)|0x7);
    //current protection to 240ma
    writeReg(RegOcp, 0x20 | 27);
...
void radio_irq_handler (u1_t dio) {
    ...
    if( (readReg(RegOpMode) & OPMODE_LORA) != 0) { // LORA modem
        ...
        if( flags & IRQ_LORA_TXDONE_MASK ) {
            ...
            LMIC.txend = now - us2osticksRound(43); // TXDONE FIXUP
            ...
            //LMIC.rssi = radio_rssi() - 157 + RSSI_OFF;
            LMIC.snr  = readReg(LORARegPktSnrValue); // SNR [dB] * 4
            LMIC.rssi = readReg(LORARegPktRssiValue) - 125 + 64; // RSSI [dBm] (-196...+63)
#if LMIC_DEBUG_LEVEL > 0
        lmic_printf("%lu: SNR:%d/4 RSSI: %d\n", now, ((int16_t)LMIC.snr), (((int16_t)readReg(LORARegPktRssiValue)) - 137));
#endif     
